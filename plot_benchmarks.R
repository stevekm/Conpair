...
colnames(df) <- c("num_threads", "time", "num_pairs", "num_tumors", "num_normals")
...
ggplot(data = df, aes(x = num_pairs, y = time/60, color = num_threads)) + 
    geom_line() +
    geom_point() + 
    theme_bw() +
...
ggplot(data = df, aes(x = num_threads, y = time_per_pair)) +
    geom_boxplot() +
    ggtitle("Conpair Time Per Pair vs. Thread Count") +
    theme_bw()
dev.off()
